...
precision_kok <- function(m, nboot, extrayears){
  ...
  if (class(m) == "glmmTMB") {
    cis <- confint(m, method = 'wald')
    cis <- subset(cis, rownames(cis) %in%  attr(cis, 'dimnames')[[1]][grepl(x = attr(cis, 'dimnames')[[1]], pattern = ':')])
    
    
    meaneffects <- subset(coef(summary(m))$cond,
                          rownames(coef(summary(m))$cond) %in%  attr(coef(summary(m))$cond  , 'dimnames')[[1]][grepl(x = attr(coef(summary(m))$cond, 'dimnames')[[1]], pattern = ':period')]
                          ,select = Estimate)
  ...
  try(
    for (ey in unique(extrayears)) {
      for (i in 1:nboot) {
        ...
        if (class(m) == "glmmTMB") {
          newcis <- confint(newm, method = 'wald')
          newcis <- subset(newcis, rownames(newcis) %in%  attr(newcis, 'dimnames')[[1]][grepl(x = attr(newcis, 'dimnames')[[1]], pattern = ':')])
